...
export class AdminService {
  constructor(
    ...
    private readonly mailService: MailService, // private readonly jwtService:JwtService,
  ) {}
  ...
  async login(loginUserDto: LoginAdminDto, res: Response) {
    ...
    const isPass = await bcrypt.compare(password, admin.hashed_password);
    if (!isPass) {
  ...
  //     };
  //   }
  //   const users = await Admin.findAll({ where });
  //   if (!users) {
